---
title: "raiku config — View and Modify CLI Configuration"
sidebarTitle: "config"
description: "raiku config manages settings in ~/.raiku/config.toml via list, get, set, and reset subcommands. Controls safe mode, verbosity, color, cache path, and remote URLs."
---
...
| `cache_dir` | string | `~/.raiku/cache` | Absolute path to the local package cache directory. |
...
```bash Move cache to a larger drive
raiku config set cache_dir /mnt/ssd/raiku-cache
```
